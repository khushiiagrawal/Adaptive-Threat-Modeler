...
function OrganicShapes() {
  ...
  const createOrganicShape = (offset: number) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      let radius = 1 + Math.sin(phi * 3 + offset) * 0.3 + Math.sin(theta * 4 + offset) * 0.2;
  ...
  useFrame((state) => {
    ...
    if (leftRef.current && rightRef.current) {
      ...
      const leftPositionsArray = leftRef.current.geometry.attributes.position.array as Float32Array;
      const rightPositionsArray = rightRef.current.geometry.attributes.position.array as Float32Array;
      
      for (let i = 0; i < particleCount; i++) {
        const i3 = i * 3;
        
        // Add flowing motion
        leftPositionsArray[i3] = leftPositions[i3] + Math.sin(time + i * 0.01) * 0.1;
        leftPositionsArray[i3 + 1] = leftPositions[i3 + 1] + Math.cos(time + i * 0.02) * 0.1;
        
        rightPositionsArray[i3] = rightPositions[i3] + Math.sin(time + i * 0.01 + Math.PI) * 0.1;
        rightPositionsArray[i3 + 1] = rightPositions[i3 + 1] + Math.cos(time + i * 0.02 + Math.PI) * 0.1;
      }
      ...
      <div ref={ref} className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          >
            ...
            for a comprehensive security assessment and tailored protection strategy.
          </motion.p>
