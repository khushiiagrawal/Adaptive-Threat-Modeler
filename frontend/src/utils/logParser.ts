...
export class LogParser {
  static parse(rawLogs: string): ParsedLogs {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      } else if (line.includes("Files Scanned:")) {
        const filesMatch = line.match(/Files Scanned:\s*(\d+)\/(\d+)/);
        if (filesMatch) {
          metadata.filesScanned = parseInt(filesMatch[1]);
          metadata.totalRepoFiles = parseInt(filesMatch[2]);
        }
      }
          ...
          if (section.data.results && Array.isArray(section.data.results)) {
            for (const result of section.data.results) {
              vulnerabilities.push({
                ...
                description:
                  result.extra?.message || "No description available",
                file: result.path,
